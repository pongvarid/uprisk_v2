...
class Report6Module extends VuexModule {
  ...
  public async loadENV() {
    ...
    this.courses = await Core.getHttp(
      `/api/cause/?risk=${this.riskID}&risk_type=6 `
    );
    this.effects = await Core.getHttp(
      `/api/effect-full/?risk=${this.riskID}&risk_type=6 `
    );
    this.controls = await Core.getHttp(
      `/api/control/?risk=${this.riskID}&risk_type=6 `
    );
    this.kris = await Core.getHttp(`/api/kri/?risk=${this.riskID}&risk_type=6 `);
    ...
    this.maesures = await Core.getHttp(`/api/maesures/?effect__risk=${this.riskID}&rm_type=1`)
    let all = await Core.getHttp(`/api/overall/?risk=${this.riskID}&rm_type=6`)
    if (all.length > 0) {
      this.all = all[0];
    }
  ...
  public async openReport(rmId: Number) {
    ...
    this.docDefinition = {
      ...
      content: [
        // header 1 
        { text: "(RM-6)", fontSize: 12, bold: true, alignment: 'right', margin: [0, 10, 10, 5], },
        { text: "แผนบริหารจัดการความเสี่ยงและควบคุมภายใน (RM-6) รอบ 6 เดือน  ประจำปีงบประมาณ พ.ศ. " + this.year.year, fontSize: 16, bold: true, alignment: 'center' },
        ...
        },
        {
          ...
          table: {
            body: [
              [{ text: "ระดับความเสี่ยงก่อนการบริหารจัดการปัจจุบัน", alignment: 'center', fontSize: 9 },
              { text: 'การบริหารจัดการความเสี่ยง', alignment: 'center', fontSize: 9 },
              { text: 'ผลการดำเนินงาน รอบ 6 เดือน', alignment: 'center', fontSize: 9 },
              { text: 'สรุปผลการบริหารความเสี่ยง และการควบคุมภายใน \n รอบ 6 เดือน (ภาพรวม)', alignment: 'center', fontSize: 9 },
              { text: 'การประเมินผลตามดัชนีชี้วัด\n (KRI) และ (KPI)\n รอบ 6 เดือน', alignment: 'center', fontSize: 9 },
              { text: 'ระดับความเสี่ยงหลังการบริหารจัดการ ที่คงเหลือหลังการจัดการความเสี่ยง รอบ 6 เดือน', alignment: 'center', fontSize: 9 },
              ],
              [{ text: this.risk.rate_full0, fontSize: 9 }, 
        ...
        { text: `ผลการประเมิน : ${this.rate6.xresult}`, fontSize: 12, margin: [30, 10, 20, 5] },
        {
...
export const Report6 = new Report6Module({ store, name: "Report6" })
