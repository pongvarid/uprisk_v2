...
class ReportModule extends VuexModule {
  ...
  public async loadENV() {
    ...
    this.courses = await Core.getHttp(
      `/api/cause/?risk=${this.riskID}&risk_type=1 `
    );
    this.effects = await Core.getHttp(
      `/api/effect/?risk=${this.riskID}&risk_type=1 `
    );
    this.controls = await Core.getHttp(
      `/api/control/?risk=${this.riskID}&risk_type=1 `
    );
    this.kris = await Core.getHttp(`/api/kri/?risk=${this.riskID}&risk_type=1 `);
    let risk_cal = await Core.getHttp(
      `/api/riskdata/?risk=${this.riskID}&rm_type=1`
    );
  ...
  public async openReport(rmId: Number) {
    ...
    this.docDefinition = {
      ...
      content: [
        // header 1 
        { text: "(RM-Plan)", fontSize: 12, bold: true, alignment: 'right', margin: [0, 10, 10, 5], },
        { text: "แผนบริหารจัดการความเสี่ยงและควบคุมภายใน (RM-Plan)  ประจำปีงบประมาณ พ.ศ. " + this.year.year, fontSize: 16, bold: true, alignment: 'center' },
        ...
        },
        {
          ...
          table: {
            body: [
              [{ text: "สาเหตุ/ปัจจัยเสี่ยง", alignment: 'center', fontSize: 9  },
              { text: 'การควบคุมภายในที่มีอยู่ในปัจจุบัน', alignment: 'center', fontSize: 9  },
              { text: 'ผลกระทบของความเสี่ยง และมาตรการจัดการความเสี่ยง', alignment: 'center', fontSize: 9  },
              { text: 'ดัชนีชี้วัดความเสี่ยง (KRI) และดัชนีชี้วัดความสำเร็จ (KPI)', alignment: 'center', fontSize: 9  },
              { text: 'ระดับความเสี่ยงก่อนการบริหาร จัดการปัจจุบัน', alignment: 'center', fontSize: 9  },
              { text: 'กำหนดเสร็จ/ผู้รับผิดชอบ', alignment: 'center', fontSize: 9  },
              ],
              [{ text: this.genCause(), fontSize: 9 }, {text:this.genControls(), fontSize: 9}, {text:this.genEffects() , fontSize: 9} , {text:this.genKris() , fontSize: 9}, {text:this.risk.rate_full0,fontSize:9}, {text:`${this.risk.assign_name}\n${this.genDate(this.risk.assign_date)}`,fontSize:9}],
        ...
        { text: `ผลการประเมิน : ${this.rate0.xresult}`, fontSize: 12, margin: [30, 10, 20, 5] },
        {
...
export const Report = new ReportModule({ store, name: "Report" })
