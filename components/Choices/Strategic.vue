  ...
  <div class="p-6">
    <div class="flex flex-col md:flex-row">
      <div class="w-full md:w-1/3">
        <h2 class="mt-4 text-xl">ยุทธศาสตร์ ที่มหาวิทยาลัยกำหนด</h2>
      ...
      <div class="w-full md:w-2/3">
        <h2 class="mt-4 text-xl">ยุทธศาสตร์ ของหน่วยงาน {{user.agency_name}}</h2><br>
        <form @submit.prevent="saveData()" class="flex">
          <v-text-field required v-model="form.name" dense outlined label="ระบุยุทธศาสตร์"></v-text-field>
...
export default {
  ...
  methods:{
    async loadData(){
      this.upChoices = _.filter(Core.choices.strategic,{agency:21})
      this.agencyChoices = _.filter(Core.choices.strategic,{agency:this.user.agency_id})
    },
    async saveData(){
      this.form.agency = this.user.agency_id
      let save = await Core.postHttp(`/api/strategic/`,this.form)
      if(save.id){
        this.form ={}
    ...
    async deleteData(id){
      let data = await Core.deleteHttp(`/api/strategic/${id}/`)
      await Core.getRMEnv();
      await this.loadData();
    }
  },
