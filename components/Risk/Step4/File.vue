...
export default {
    ...
    methods: {
        async check() {
            ...
            this.listFile = await Core.getHttp(`/api/conclusion-file/?risk=${this.risk.id}&rm_type=${this.current_type}`)
            await Web.switchLoad(false)
            this.response = true;
        },

        async removeFile(id){
            await Core.deleteHttpAlert(`/api/conclusion-file/${id}/`)
            await this.check();
        },
        ...
        async updateFile() {
            ...
            let data = await Core.postHttpAlert(`/api/conclusion-file/`, formData)
            this.file = null
            await Web.switchLoad(false)
            await this.check();
        },
