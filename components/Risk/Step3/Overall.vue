...
<div><br>
    <v-card outlined class="p-6">
        <form @submit.prevent="update(form)" v-if="response">
            ...
            <Risk-Step3-File class="mb-6" :risk="risk" :current_type="current_type" :rm_type="rm_type"></Risk-Step3-File>
...
export default {
    ...
    methods: {
        async check() {
            this.response = false;
            let data = await Core.getHttp(`/api/overall/?risk=${this.risk.id}&rm_type=${this.current_type}`)
            await Web.switchLoad(true)
            console.log(data)
            if (data.length === 0) {
                let save = await Core.postHttp(`/api/overall/`, { "risk": this.risk.id, "rm_type": this.current_type })
                this.form = save
        ...
        async update(form) {
            await Core.putHttpAlert(`/api/overall/${form.id}/`, form)
            await this.check();
        },
