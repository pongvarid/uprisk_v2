<template>
<div>
    กรุณาเลือก สังกัด หน่วยงานของคุณ
    <v-autocomplete prepend-inner-icon="mdi-office-building" filled   label="สังกัด หน่วยงาน" v-model.number="formReg.agency" item-value="id" item-text="name"   :items="agencies"></v-autocomplete>
    <v-btn @click="updateAgency()" color="success">ยืนยัน</v-btn>
...
export default class Root extends Vue {
    ...
    async updateAgency() {
        ...
        let user = await Core.putHttpAlert(`/api/user/${this.user.id}/`, this.formReg)
        if(user.id){
