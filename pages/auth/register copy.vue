<template>
  <div class=" flex justify-center items-center p-2 bgh">
    <v-card class="w-full md:w-1/2 rounded-xl elevation-8">
      <v-card-title>
        <v-icon @click="$router.go('-1')">mdi-arrow-left</v-icon>

        <h2 class="ml-2 text-xl font-bold text-purple-800">ลงทะเบียนเข้าใช้งาน </h2>
      </v-card-title>
      <v-card-text>
        <div v-if="errorReg">
        ...
        <form @submit.prevent="registerData()">
          <v-text-field   required v-model="formReg.first_name" filled rounded label="ชื่อ" prepend-inner-icon="mdi-account-box"></v-text-field>
          <v-text-field   required v-model="formReg.last_name" filled rounded label="สกุล" prepend-inner-icon="mdi-account-box"></v-text-field>

          <v-text-field   required v-model="formReg.username" filled rounded label="ชื่อผู้ใช้" prepend-inner-icon="mdi-face"></v-text-field>
          ...
          <v-autocomplete prepend-inner-icon="mdi-office-building" filled rounded label="สังกัด หน่วยงาน" v-model="formReg.agency" item-value="id" item-text="name" e :items="agencies"></v-autocomplete>
          <v-btn v-if="formReg.password == formReg.password_confirm && formReg.password != ''" rounded large color="success" class="w-full" type="submit">ลงทะเบียน</v-btn>
        </form>
      </v-card-text>
