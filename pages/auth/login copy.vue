  ...
  <div class="bgh">
    <div class="h-screen flex justify-center items-center p-2">
      <v-card class="w-full md:w-1/2 rounded-xl elevation-8">
        <v-card-title>
          <p class="text-xl font-bold text-purple-800">เข้าสู่ระบบ<br> <span class="text-base font-thin">ระบบบริหารจัดการความเสี่ยง</span></p>
          <br>
        </v-card-title>
        <v-card-text>
          <form @submit.prevent="login()">
            <v-text-field required v-model="form.username" filled label="ชื่อผู้ใช้" rounded prepend-inner-icon="mdi-form-textbox-password"></v-text-field>
            ...
            <v-btn dark depressed large color="purple" class="w-full" type="submit" rounded>เข้าสู่ระบบ</v-btn>
            <br><br>
            <v-btn text @click="$router.push('/auth/register')" large rounded color="purple" class="w-full font-bold">ลงทะเบียน</v-btn>
          </form>
        </v-card-text>
