<template>
  <v-dialog v-model="dialog" fullscreen persistent >
    <div class="p-6 bg-white h-screen">
      ...
      <center>
        <h2 class="ff text-base  font-bold">แผนการจัดการความเสี่ยงและควบคุมภายใน (RM-6) รอบ 6 เดือน </h2>
      ...
      <table class="w-full ff">
        ...
        <tr class="w-full">
          <td>สาเหตุ </td>
          <td>การดำเนินการจัดการความเสี่ยง (รอบ 6 เดือน)</td>
          <td>ผลการดำเนินงาน (รอบ 6 เดือน)</td>
        ...
        <tr class="w-full">
          ...
          <td style="width:15%;">
            {{risk_cal.chance}} x {{risk_cal.result}} = {{risk_cal.xscore}}
      ...
      <div class="flex flex-row justify-center items-center">
        ...
        <div class="w-1/4"><br><br>
          <img :src="risk.sign_img6" alt="">
          <h2 class="ff">ลายมือชื่อ...................................</h2>
          <h2 class="ff">({{risk.sign_name6}})</h2>
          <h2 class="ff">ตำแหน่ง{{risk.sign_position6}}</h2>
          <h2 class="ff">{{core.convertDate(risk.sign_date6)}}</h2>
        </div>
      </div>
...
export default {
  ...
  methods:{
    ...
    async loadENV(){
      ...
      this.effects = await Core.getHttp(`/api/effect/?risk=${this.id}&risk_type=6 `)
      this.controls = await Core.getHttp(`/api/control/?risk=${this.id}&risk_type=6 `)
      this.kris = await Core.getHttp(`/api/kri/?risk=${this.id}&risk_type=6 `)
